...
var HPC = function(host, port, ident, secret, strChannels, fnMsg) {
	var self = this;
	...
	this.handle = function(opcode, data) {
		if (opcode == OP_INFO) {
			...
			//console.log('handle: ',data.toString());
			
			ready = true;

			self.connectcb && self.connectcb();
			conn.write(msgauth(rand, ident, secret));
			//subscribe to all channels given
			var arrChannels=strChannels.split(',');
			arrChannels.forEach( function(v,k){ self.subscribe(v); });
